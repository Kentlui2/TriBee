    ...
    <div class="flex flex-col lg:flex-row gap-8">
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <div class="flex items-center gap-3 mt-3">
                            ...
                            <form action="{{ route('cart.update', $item['cart_item_id']) }}" method="POST" class="flex items-center gap-0.5">
                                @csrf
                                @method('PUT')
                                <button type="button" onclick="var i=this.nextElementSibling;var q=Math.max(1,parseInt(i.value)-1);i.value=q;this.form.querySelector('[name=quantity]').value=q;this.form.submit();" class="w-7 h-7 bg-gray-100 hover:bg-gray-200 rounded-lg text-xs font-bold">−</button>
                                <input type="text" value="{{ $item['quantity'] }}" disabled class="w-10 text-center bg-transparent border-0 text-sm font-semibold text-neutral-800 cursor-default p-0">
                                <button type="button" onclick="var i=this.previousElementSibling;var q=parseInt(i.value)+1;i.value=q;this.form.querySelector('[name=quantity]').value=q;this.form.submit();" class="w-7 h-7 bg-gray-100 hover:bg-gray-200 rounded-lg text-xs font-bold">+</button>
        ...
        <div class="lg:w-1/3">
            <div class="sticky top-8 space-y-6">
                ...
                <a href="{{ route('checkout.shipping') }}" class="block w-full py-4 bg-gradient-to-r from-orange-500 to-orange-600 text-white font-bold rounded-2xl text-center hover:opacity-90 transition shadow-lg shadow-orange-500/20">
                    Proceed to Checkout
                </a>
            </div>
        </div>
